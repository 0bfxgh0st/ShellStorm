...
printf "\n[${COL_BLUE}+${COL_END}] war file compiled in $PWD/payload.war\n"
...
#	printf "[ ${COL_BLUE}C${COL_END} ] [(reverse)]\n\n"
	_c_ "$1" "$2" "$3"
        gcc Cprog.c -o revshell
        cat Cprog.c
	printf "[${COL_BLUE}+${COL_END}] Binary file is already compiled and stored in $PWD/revshell\n"
...
then
#       printf "[ ${COL_BLUE}RUST${COL_END} ] [(reverse)]\n\n"
        _rust_ "$1" "$2" "$3"
	cd /tmp/build
	cargo build
